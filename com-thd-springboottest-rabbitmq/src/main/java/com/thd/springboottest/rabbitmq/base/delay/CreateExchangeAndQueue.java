...
public class CreateExchangeAndQueue {
    public static void main(String[] args) throws Exception{
        ...
        String QUEUE_NAME = "normal-queue";// 普通队列名称
        String ROUTE_KEY = "normal-routekey";// 普通路由键名称
        String EXCHANGE_NAME = "normal-exchange";// 普通交换器名称

        String DELAY_QUEUE_NAME = "delay-queue";// 延时队列名称
        String DELAY_ROUTE_KEY = "delay-routekey";// 延时路由键名称
        String DELAY_EXCHANGE_NAME = "delay-exchange";// 延时交换器名称
        ...
        // ====================== 创建私信队列、交换机、并绑定 ===================
        // 创建一个延时队列(就是一个普通的队列，其实也是一个死信队列)
        channel.queueDeclare(DELAY_QUEUE_NAME, true, false, false, null);
        // 创建一个死信交换机
        channel.exchangeDeclare(DELAY_EXCHANGE_NAME, BuiltinExchangeType.DIRECT, true, false, false, null);
        // 绑定死信队列和私信交换机
        channel.queueBind(DELAY_QUEUE_NAME, DELAY_EXCHANGE_NAME, DELAY_ROUTE_KEY);
        ...
        arguments.put("x-dead-letter-exchange" , DELAY_EXCHANGE_NAME); // 设置死信交换机的名称
        arguments.put("x-dead-letter-routing-key", DELAY_ROUTE_KEY); // 这个DLX指定routing key，如果没有特殊指定，则使用原队列的routing key
